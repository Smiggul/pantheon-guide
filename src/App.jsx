...
useEffect(() => {
  ...
  // (incl. support items) resolves to an icon. item.image is "3071.png" — strip
  // the extension; keep the first (lowest-id = base) item when Ornn Masterwork
  // upgrades share a name.
  fetch("/ddragon/data/item.json")
    ...
    .then(json => {
      ...
      setItemMap(map);
      // Build Forge picker: only offer items actually purchasable on Summoner's
      // Rift (sr-items.json allowlist, generated by helpers/gen-sr-items.mjs) so
      // Arena-only / champion-locked / non-Rift items never show. Fall back to
      // the full item set if the allowlist isn't present.
      fetch("/ddragon/data/sr-items.json")
        .then(r => (r.ok ? r.json() : Promise.reject()))
        .then(sr => setItemList(sr.items))
        .catch(() => setItemList(
          Object.entries(map)
            .map(([key, id]) => ({ name: Object.values(json.data).find(i => i.name.toLowerCase() === key)?.name || key, id }))
